...
class verifyAccount extends Component {
  ...
  componentDidMount() {
    if (this.props.match.params.userId !== "send") {
      app
        .post("/user/verify/" + this.props.match.params.userId)
        .then(() => {
          this.setState({
            verified: true,
          });
  ...
  resendVerifyEmail() {
    ...
    app
      .get(`/user/getUserInfo/${this.props.userSession.userId}`)
      .then((res) => {
        const data = {
        ...
        app
          .post("/accountVerification/sendVerificationEmail", data, {
            headers: {
      ...
      <div className="container">
        {this.props.match.params.userId !== "send" ? (
          ...
          <div>
            ...
            Click the button below to resend a verification link <br />
            <br />
            <input
              type="button"
            ...
            <br />
            <br />
            {this.state.loading ? <div id="spinner"></div> : null}
            {this.state.sent ? (
              <div>
                <h3>Verification has been sent! Please check your email.</h3>{" "}
...
export default withRouter(connect(mapStateToProps)(verifyAccount));
