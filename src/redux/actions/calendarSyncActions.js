...
export const importCalendar = (calendarURL, listingId) => async (dispatch) => {
  ...
  await axios
    ...
    .then(async (res) => {
      ...
      if (
        availableStart.length > 0 &&
        availableEnd.length > 0 &&
        availableStart.length === availableEnd.length
      ) {
        ...
        for (let i = 0; i < availableStart.length; i++) {
          if (!listingId) {
            if (
              availableEnd[i].getTime() === earliestEnd.getTime() ||
              availableStart[i].getTime() === latestStart.getTime()
            ) {
              continue;
            }
          }
          ...
          booked.push({
            start: bookedStart
              .toISOString()
              .substring(0, availableStart[i].toISOString().indexOf("T")),
            end: bookedEnd
              .toISOString()
              .substring(0, availableEnd[i].toISOString().indexOf("T")),
            reservationId: null,
          });
        }
        if (!listingId) {
          ...
          await app
            .put("/listings/syncListing/" + listingId, data)
            .catch(() => {
              alert("Unable to sync calendar.");
            });
        }
