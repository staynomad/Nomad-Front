...
export const UPDATE_USER_RESERVATIONS = 'VHomes/reservation/UPDATE_USER_RESERVATIONS';

/* Actions */
const setUserReservations = (reservations) => ({ type: SET_USER_RESERVATIONS, reservations });
const updateUserReservations = (reservation) => ({ type: UPDATE_USER_RESERVATIONS, reservation });
...
export const checkInToReservation = (token, reservationId) => async dispatch => {
    dispatch(setLoadingTrue());
    const headers = { "Authorization": `Bearer ${token}` };
    const checkInRes = await handleReq(`/reservation/activate/${reservationId}`, "POST", headers)
...
export const checkOutOfReservation = (token, reservationId) => async dispatch => {
    dispatch(setLoadingTrue());
    const headers = { "Authorization": `Bearer ${token}` };
    const checkOutRes = await handleReq(`/reservation/deactivate/${reservationId}`, "POST", headers)
